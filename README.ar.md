...
- [اسأل الدورة والأسئلة التلقائية](#اسأل-الدورة-والأسئلة-التلقائية)
...
- 💬 **اسأل الدورة والأسئلة التلقائية**: اسأل سؤالاً عن دورة كاملة واحصل على الإجابة مع رقم الدرس والدقيقة، وأسئلة اختيار من متعدد وصح/خطأ لكل درس.
...
**مفتاح مجاني مدمج:** يمكن أن يأتي التطبيق مع مفتاح Groq فيعمل الملخص والأسئلة و«اسأل الدورة» دون أي إعداد (النموذج الافتراضي `openai/gpt-oss-120b`). ضع المفتاح في ملف `builtin-keys.json` في جذر المشروع بهذا الشكل: `{"cloud:groq": "gsk_…"}`. هذا الملف لا يُرفع إلى git ويُنسخ داخل ملف التثبيت. وإذا حفظ المستخدم مفتاحه الخاص في الإعدادات يُستخدم مفتاحه أولاً. انتبه: أي شخص لديه ملف التثبيت يستطيع استخراج هذا المفتاح، وكل المستخدمين يتشاركون حدود Groq الخاصة به.
---

## اسأل الدورة والأسئلة التلقائية

تستخدم الميزتان نفس المزوّد والمفتاح المستخدمين في [الملخص الذكي](#الملخص-الذكي-والفصول)، ويُرسل النص فقط.
...
**أسئلة على التفريغ:** في آخر الصفحة تحت الملخص، من **أسئلة على التفريغ ← إنشاء الأسئلة** تُكتب أسئلة اختيار من متعدد و/أو صح وخطأ (من 5 إلى 20) عن أهم أفكار التفريغ. اضغط على أي خيار لتعرف إن كان صحيحاً، فتظهر الإجابة الصحيحة وشرح قصير ورابط للدقيقة التي يُشرح فيها. تُحفظ الأسئلة مع التفريغ في الأرشيف، ويمكن نسخها نصاً، وهي متاحة لقاعدة البيانات عبر `@quiz` و `@quiz_json`.
...
| لكل صف | للملف |
|---|---|
| `translation`: النص المترجم لنفس الفترة الزمنية، و `text_en`: نفسه إذا كانت الترجمة إنجليزية | `full_translation` و `full_text_en` و `translation_language` و `summary` و `key_points` (نقطة في كل سطر) و `chapters` (`00:05:12 العنوان` في كل سطر) و `chapters_json` و `keywords` (مفصولة بفواصل) و `quiz` (أسئلة التفريغ نصاً) و `quiz_json` و `course` |
...
- **الملخص الذكي والأسئلة و«اسأل الدورة»** تُرسل نص التفريغ فقط (وليس الصوت) إلى المزوّد الذي اخترته، وفقط عندما تطلبها.
