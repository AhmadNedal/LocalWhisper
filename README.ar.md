...
(غيّر `version` في `package.json` قبلها.) يمكن أيضاً تشغيله يدوياً من تبويب **Actions** ← **Windows installer** ← **Run workflow**، فيظهر الملف تحت **Artifacts**. لتضمين مفتاح Groq المجاني أضفه في المستودع: **Settings ← Secrets and variables ← Actions** باسم `GROQ_API_KEY`.
